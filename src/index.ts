...
program.command("build")
  ...
  .option("-q, --quiet", "do not log anythin", false)
  .action(async (opts: BuildOpts) => {
    new BuildAction(opts.watch, opts.clean, await getCurrentDir(), !opts.quiet).run()
  })
...
program.command("serve")
  ...
  .option(cleanOpt[0], cleanOpt[1])
  .option("-v, --verbose", "log anything", false)
  .action(async (opts: ServeOpts) => {
    new ServeAction(opts.clean, await getCurrentDir(), opts.verbose).run()
  })
