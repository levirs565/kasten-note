...
function analyzerAttacker(
  doc: document.Options,
  onImageFound: ImageFoundListener
) {
  return analyzer;
  ...
  function analyzer(tree: unist.Node, file: vfile.VFile) {
    ...
    function visitor(
      ...
      if (node.type == 'heading') {
        ...
        if (image.url.indexOf('://') >= 0) return;

        let imgPath = image.url;
        if (imgPath[0] == '/') imgPath = imgPath.substring(1);
        else imgPath = path.join(file.dirname!!, imgPath);
...
export async function buildMarkdown(
  input: vfile.VFile,
  wikiLinkResolver: (name: string) => (string | undefined)[],
  onImageFound: ImageFoundListener
) {
  ...
  const resultFile = await processor.process(input)
  return resultFile
}
